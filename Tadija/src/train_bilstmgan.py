from models import DCDiscriminator,BiLSTMGenerator
...
def train_bilstmgan(batch_size,train_steps,model_dir,heartbeat_type,use_simulator=True):
	...
	val_noise=torch.Tensor(np.random.normal(0,1,(4,216*8)))
	steps=0
	while True:
		for i,batch in enumerate(data_loader):
			...
			noise=torch.Tensor(np.random.normal(0,1,(size,216*8)))
			...
			print("Step {} done. (BiLSTMGAN, heartbeat type {})".format(steps,heartbeat_type))
...
if __name__=="__main__":
	train_bilstmgan(200,2000,"GAN\\LSTM\\BiLSTMGAN\\00_N","N",use_simulator=False)
