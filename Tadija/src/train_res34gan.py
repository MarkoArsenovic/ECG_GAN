from models import Res34Generator,Res34Discriminator
...
def train_res34gan(batch_size,train_steps,model_dir,heartbeat_type,use_simulator=True,continue_from=-1):
	generator=Res34Generator()
	generator.apply(initialize)
	discriminator=Res34Discriminator()
	...
	if continue_from!=-1:
		chkp=torch.load(os.path.join(model_dir,'training_checkpoint_'+str(continue_from)))
		generator.load_state_dict(chkp['generator_state_dict'])
		discriminator.load_state_dict(chkp['discriminator_state_dict'])
		g_optimizer.load_state_dict(chkp['optimizer_g_state_dict'])
		d_optimizer.load_state_dict(chkp['optimizer_d_state_dict'])
		steps=continue_from
	...
	while True:
		for i,batch in enumerate(data_loader):
			...
			labels=torch.full((size,1),1,device='cpu').float()
			...
			labels=torch.full((size,1),0,device='cpu').float()
			...
			print("Step {} done. (Res34GAN, heartbeat type {})".format(steps,heartbeat_type))
...
if __name__=="__main__":
	train_res34gan(200,10000,'GAN\\ResGAN\\Res34GAN\\00_N',"N",use_simulator=False,continue_from=750)